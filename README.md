
# Temas relacionado al proyecto de NORTHSTAR

- [Como deployar BPMN a PP sin pasar por el proceso normal de GitHub](./src/manual_deploy_bpmn.md)

- [Ejecutar un script de updates relacionado a Document Tracking Report Clean Up](./src/manual_execution_script.md)

## Proyecto de Soporte ( viejo )

- [Compilar backend](./src/compilar_backend.md)

- [Compilar proyecto en particular del backend](./src/compilar_proyecto_particular.md)

## Chusmear y hacer cosas en Docker

- [Entrar a docker]

- [Editar archivo desde afuera]

- [Restart todos los containers]